...
- **`inventario.csv`**: requeridos `sku, existencia` (existencia a nivel SKU, no por CD). `pack` y `lead_time_dias` son **opcionales**: si faltan (o si el SKU no está en el archivo) se rellenan con `INV_DEFAULTS` = 1 y 30 días, pisables desde la UI vía `carga.json["defaults"]`. Ojo: un lead time inventado produce un `estado_inventario` inventado.

  **Trampa de escala** (costó una sesión entera de debug): `existencia` tiene que estar en las **mismas unidades** que `cantidad` de ventas. Si el archivo de inventario viene en otra escala (cajas vs unidades, snapshot de una sola tienda, muestra parcial), todos los KPIs salen basura pero *sin error* — DOH de horas, el 100% de los SKUs en Riesgo de quiebre. Peor: `prorratear_existencia` divide esa existencia entre los CDs del SKU, así que un número ya pequeño se vuelve fracción (`0.03`) y la UI lo muestra como **`0`** por el `.round(0)` de las tablas. Antes de culpar al código, comparar existencia total contra ventas mensuales totales: si el stock no cubre ni unos pocos días de demanda, el problema son los datos.
